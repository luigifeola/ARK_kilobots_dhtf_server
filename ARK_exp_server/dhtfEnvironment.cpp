...
namespace
{
    ...
    const double reachable_distance = (ARENA_SIZE * SCALING / 2.0) - (2.0 * KILO_DIAMETER);
...
{
    double white_space = SCALING * 2.0 * KILO_DIAMETER;
...
void mykilobotenvironment::updateVirtualSensor(Kilobot kilobot_entity)
{
    ...
    if (this->time - this->lastSent[k_id] > minTimeBetweenTwoMsg)
    {
                 ...
                 abs(shifted_pos.y()) > reachable_distance)
        {
            ...
            proximity_decimal = std::accumulate(proximity.begin(), proximity.end(), 0, [](int x, int y)
                                                { return (x << 1) + y; });
